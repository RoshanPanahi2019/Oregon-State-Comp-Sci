...
# dummies_grade=pd.get_dummies(train.grade)
# dummies_condition=pd.get_dummies(train.condition)
# dummies_view=pd.get_dummies(train.view)
# dummies_floors=pd.get_dummies(train.floors)


train[["mm","day" ,"year"]] = train["date"].str.split("/", expand=True)
dummies_year=pd.get_dummies(train.year)
dummies_month=pd.get_dummies(train.mm)
#print(train[["mm","day" ,"year"]])
train["mm"] = train["mm"].astype(int)
train["day"] = train["day"].astype(int)
train["year"] = train["year"].astype(int)
#train=pd.concat([train,dummies_grade,dummies_condition,dummies_view,dummies_floors,dummies_month,dummies_year],axis='columns')
train=pd.concat([train,dummies_month,dummies_year],axis='columns')




#Normalize
X_df=train.drop(columns=['dummy','id','date','price','zipcode','lat','long'])
#print(X_df)
...
k=np.dot(Weight,(np.transpose(X_norm)))
k=np.transpose(k)
#print(k.shape)
#print(sum(k-Y))
