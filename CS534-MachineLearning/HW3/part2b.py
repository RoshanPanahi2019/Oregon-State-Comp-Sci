    ...
    while (iter<maxiter):
        ...
        for i in range(m): 
            ...
            for j in range(m):  
                ...
                u=u+a
            Y_hat_train.append(u)
        
        for i in range(m):        
            if (Y_hat_train[i]*Y[i,0]<=0) :
                alpha[0,i]=alpha[0,i]+lr 
            else:
                correct+=1  
        ...
        for i in range(m_val):   
            ...
            for j in range(m):      
                ...
                u_val=u_val+a_val
            Y_hat_val.append(u_val)
            
        for i in range(m_val):    
            if (Y_hat_val[i]*Y_Val[i,0]>0):
                correct_val+=1      
...
if __name__ == "__main__": 
    ...
    Result_best_train=[.835,.88,.895,.935]
    Result_best_val=[.7,.7,.7,.7]
    P_list=[10,20,30,40]
    
    myplot_1(accuracy)
    myplot_1(accuracy_val)
    
    myplot_2(Result_best_train,P_list,"Iterations")
